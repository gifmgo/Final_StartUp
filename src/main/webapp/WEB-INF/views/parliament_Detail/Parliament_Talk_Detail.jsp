          ...
          <c:when test="${detail ne null}">
                ...
                <%-- 첨부파일 :<a href="paliament_talk_fileDown.do?name=${detail.filename}">${detail.filename}</a>  --%>
                     <div id="title">
                      <h2>${detail.title}&nbsp;&nbsp;&nbsp;<span class="nick">작성자 : ${detail.writer}</span></h2>
                       <p>작성일 : ${detail.writeDate}<br/> 조회수  :  ${detail.viewcnt}</p>
                   </div>
                <div id="content">
                   ${detail.content}
                </div>
               ...
               <div id="btArray">
                      <c:if test="${detail.id == sessionScope.id}">
                         ...
                         <button onclick="deleteTalk(${detail.paliament_talk_seq});">삭제</button>
                       </c:if>
                       <button onclick="location.href='PaliamentDetail.do?num=${detail.num}&dept_cd=${dept_cd}&img=${img}&name=${detail.catego}'">목록</button>
               </div>                 
          </c:when>
          <c:otherwise>
             <h4>잘못된 접근입니다!</h4>
